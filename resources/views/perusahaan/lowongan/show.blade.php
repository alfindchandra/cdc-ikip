...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
    <a href="{{ route('perusahaan.lowongan.index') }}" class="inline-flex items-center text-blue-600 hover:text-blue-700 transition-colors duration-200 font-medium group">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="bg-white rounded-xl shadow-xl border border-gray-100 p-6 space-y-8">
                ...
                <div class="border-b pb-4">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        ...
                        @php
                            ...
                            ][$lowongan->status] ?? 'bg-gray-100 text-gray-800';
                        @endphp
                        <span class="inline-flex items-center px-4 py-1.5 rounded-full text-sm font-semibold {{ $status_class }}">
                ...
                <div class="flex flex-wrap gap-3">
                    <span class="flex items-center text-sm px-3 py-1.5 font-medium bg-blue-100 text-blue-800 rounded-full">
                        <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                        {{ ucfirst(str_replace('_', ' ', $lowongan->tipe_pekerjaan)) }}
                    </span>
                    <span class="flex items-center text-sm px-3 py-1.5 font-medium bg-indigo-100 text-indigo-800 rounded-full">
                    ...
                    <span class="flex items-center text-sm px-3 py-1.5 font-medium bg-green-100 text-green-800 rounded-full">
                ...
                @foreach(['deskripsi', 'kualifikasi', 'benefit'] as $field)
                    @if($lowongan->$field)
                    <section class="space-y-3 pt-4 border-t border-gray-100">
                        <h2 class="text-xl font-bold text-gray-900 flex items-center">
                            <span class="w-8 h-8 bg-{{ ['deskripsi' => 'blue', 'kualifikasi' => 'green', 'benefit' => 'yellow'][$field] }}-50 text-{{ ['deskripsi' => 'blue', 'kualifikasi' => 'green', 'benefit' => 'yellow'][$field] }}-600 rounded-full flex items-center justify-center mr-3">
                                @if($field === 'deskripsi')
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                @elseif($field === 'kualifikasi')
                            ...
                            {{ ucfirst($field) }} Pekerjaan
                        </h2>
                        <div class="prose max-w-none text-gray-700 bg-gray-50 p-4 rounded-lg border border-gray-200 shadow-inner">
                            {!! nl2br(e($lowongan->$field)) !!}
                        </div>
                    </section>
                    @endif
                @endforeach
        ...
        <div class="lg:col-span-1 space-y-8">
            ...
            <div class="bg-white rounded-xl shadow-md border border-gray-100">
                <div class="bg-blue-50 rounded-t-xl p-4 border-b border-blue-100">
                    <h3 class="text-lg font-bold text-blue-900">Aksi Cepat</h3>
                </div>
                <div class="p-4 space-y-3">
                    <a href="{{ route('perusahaan.lowongan.edit', $lowongan->id) }}" class="w-full justify-center bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 rounded-lg transition duration-150 ease-in-out flex items-center shadow-md">
                    ...
                    <form action="{{ route('perusahaan.lowongan.status', $lowongan->id) }}" method="POST">
                        @csrf
                        <input type="hidden" name="status" value="{{ $lowongan->status == 'aktif' ? 'nonaktif' : 'aktif' }}">
                        <button type="submit" class="w-full justify-center font-semibold py-2 rounded-lg transition duration-150 ease-in-out flex items-center shadow-md 
                            {{ $lowongan->status == 'aktif' ? 'bg-yellow-500 hover:bg-yellow-600 text-white' : 'bg-green-500 hover:bg-green-600 text-white' }}">
            ...
            <div class="bg-white rounded-xl shadow-md border border-gray-100">
                <div class="bg-gray-50 rounded-t-xl p-4 border-b border-gray-100">
                ...
                <div class="p-4 space-y-4">
                    ...
                    <div class="pt-2">
                         <p class="text-sm uppercase tracking-wider text-gray-500 font-medium mb-1">Status</p>
                         <span class="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-semibold {{ $status_class }}">
                            {{ ucfirst($lowongan->status) }}
                         </span>
                    </div>
                </div>
            ...
            <div class="bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200 rounded-xl shadow-lg p-6">
