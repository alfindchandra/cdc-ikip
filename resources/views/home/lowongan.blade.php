    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div class="lg:col-span-2 space-y-3 lg:max-h-[calc(100vh-14rem)] lg:overflow-y-auto pr-1 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-gray-200 [&::-webkit-scrollbar-thumb]:rounded-full">
            <template x-for="item in lowongans" :key="item.id">
                <div>
                    <button type="button" @click="selectedId = selectedId === item.id ? null : item.id"
                ...
                <template x-for="item in lowongans" :key="item.id">
                    <div x-show="selectedId === item.id" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0">
                        ...
                        <div class="relative px-6 py-6 border-b border-gray-100 bg-gradient-to-b from-blue-50/20 to-white overflow-hidden">
                            ...
                            <div class="mt-5 grid grid-cols-2 sm:grid-cols-2 gap-3">
                                ...
                                <div class="rounded-xl bg-white border border-gray-200/80 p-3 flex flex-col items-center text-center hover:shadow-sm transition-shadow">
                                    ...
                                    <div class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Rentang Usia</div>
                                    <p class="mt-0.5 text-xs font-bold text-gray-700">20 - 40 Thn</p>
                                </div>
                                <div class="rounded-xl bg-white border border-gray-200/80 p-3 flex flex-col items-center text-center hover:shadow-sm transition-shadow">
                            ...
                            <div class="mt-4 flex items-center justify-between gap-3">
                                <a :href="item.detail_url" class="inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl bg-gray-100 text-gray-700 text-xs font-bold hover:bg-gray-200 transition">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><path d="M12 16v-4"></path><path d="M12 8h.01"></path></svg> Detail Selengkapnya
                                </a>
                                @if(Auth::check() && Auth::user()->role == 'mahasiswa')
                                <a :href="item.apply_url" class="inline-flex items-center justify-center gap-1.5 px-5 py-2 rounded-xl bg-blue-600 !text-white text-xs font-bold hover:bg-blue-700 shadow-sm transition">
